...
const Career = {
  data: null,
  load() {
    const saved = wx.getStorageSync('qb_career_v14');
  ...
  save() { wx.setStorageSync('qb_career_v14', JSON.stringify(this.data)); },
  endSeason(stats) {
    this.data.seasons++;
...
const SeedSystem = {
  ...
  copyToClipboard(text) { wx.setClipboardData({ data: text }); }
};
...
const offenseFormations = [
  ...
  // --- 30 new plays ---
  { name: '4 Verticals', getPositions: (losY) => ({ qb: { yard: losY - 5, lane: 30 },
    wrs: [{ yard: losY, lane: 5, route: 'streak' }, { yard: losY - 1, lane: 20, route: 'streak' },
          { yard: losY - 1, lane: 40, route: 'streak' }, { yard: losY, lane: 55, route: 'streak' }] })},
  { name: 'Verts Switch', getPositions: (losY) => ({ qb: { yard: losY - 5, lane: 30 },
    wrs: [{ yard: losY, lane: 5, route: 'streak' }, { yard: losY - 1, lane: 20, route: 'post' },
          { yard: losY - 1, lane: 40, route: 'post' }, { yard: losY, lane: 55, route: 'streak' }] })},
  { name: 'Streak Left', getPositions: (losY) => ({ qb: { yard: losY - 5, lane: 35 },
    wrs: [{ yard: losY - 1, lane: 6, route: 'streak' }, { yard: losY, lane: 16, route: 'streak' },
          { yard: losY - 1, lane: 24, route: 'streak' }, { yard: losY, lane: 54, route: 'flat' }] })},
  { name: 'Quick Slants', getPositions: (losY) => ({ qb: { yard: losY - 5, lane: 30 },
    wrs: [{ yard: losY, lane: 6, route: 'slant' }, { yard: losY - 1, lane: 20, route: 'slant' },
          { yard: losY - 1, lane: 40, route: 'slant' }, { yard: losY, lane: 54, route: 'slant' }] })},
  { name: 'Quick Out', getPositions: (losY) => ({ qb: { yard: losY - 5, lane: 30 },
    wrs: [{ yard: losY, lane: 6, route: 'out' }, { yard: losY - 1, lane: 20, route: 'out' },
          { yard: losY - 1, lane: 40, route: 'out' }, { yard: losY, lane: 54, route: 'out' }] })},
  { name: 'Flat/Screen', getPositions: (losY) => ({ qb: { yard: losY - 5, lane: 30 },
    wrs: [{ yard: losY, lane: 6, route: 'flat' }, { yard: losY - 1, lane: 20, route: 'curl' },
          { yard: losY - 1, lane: 40, route: 'curl' }, { yard: losY, lane: 54, route: 'flat' }] })},
  { name: 'Hitch Concept', getPositions: (losY) => ({ qb: { yard: losY - 5, lane: 30 },
    wrs: [{ yard: losY, lane: 6, route: 'hitch' }, { yard: losY - 1, lane: 20, route: 'hitch' },
          { yard: losY - 1, lane: 40, route: 'hitch' }, { yard: losY, lane: 54, route: 'hitch' }] })},
  { name: 'Quick Drag', getPositions: (losY) => ({ qb: { yard: losY - 5, lane: 30 },
    wrs: [{ yard: losY, lane: 6, route: 'drag' }, { yard: losY - 1, lane: 20, route: 'curl' },
          { yard: losY - 1, lane: 40, route: 'drag' }, { yard: losY, lane: 54, route: 'curl' }] })},
  { name: 'Mesh', getPositions: (losY) => ({ qb: { yard: losY - 5, lane: 30 },
    wrs: [{ yard: losY, lane: 6, route: 'streak' }, { yard: losY - 1, lane: 18, route: 'drag' },
          { yard: losY, lane: 42, route: 'drag' }, { yard: losY, lane: 54, route: 'post' }] })},
  { name: 'Drive', getPositions: (losY) => ({ qb: { yard: losY - 5, lane: 30 },
    wrs: [{ yard: losY, lane: 6, route: 'streak' }, { yard: losY - 1, lane: 18, route: 'drag' },
          { yard: losY - 1, lane: 38, route: 'drag' }, { yard: losY, lane: 54, route: 'seam' }] })},
  { name: 'Scissors', getPositions: (losY) => ({ qb: { yard: losY - 5, lane: 30 },
    wrs: [{ yard: losY, lane: 6, route: 'flat' }, { yard: losY - 1, lane: 16, route: 'post' },
          { yard: losY - 1, lane: 44, route: 'corner' }, { yard: losY, lane: 54, route: 'flat' }] })},
  { name: 'Cross Flow', getPositions: (losY) => ({ qb: { yard: losY - 5, lane: 30 },
    wrs: [{ yard: losY, lane: 6, route: 'drag' }, { yard: losY - 1, lane: 18, route: 'slant' },
          { yard: losY - 1, lane: 42, route: 'slant' }, { yard: losY, lane: 54, route: 'drag' }] })},
  { name: 'Flood Right', getPositions: (losY) => ({ qb: { yard: losY - 5, lane: 25 },
    wrs: [{ yard: losY, lane: 6, route: 'streak' }, { yard: losY - 1, lane: 38, route: 'out' },
          { yard: losY - 1, lane: 46, route: 'flat' }, { yard: losY, lane: 54, route: 'streak' }] })},
  { name: 'Flood Left', getPositions: (losY) => ({ qb: { yard: losY - 5, lane: 35 },
    wrs: [{ yard: losY, lane: 6, route: 'streak' }, { yard: losY - 1, lane: 14, route: 'flat' },
          { yard: losY - 1, lane: 22, route: 'out' }, { yard: losY, lane: 54, route: 'streak' }] })},
  { name: 'Levels Right', getPositions: (losY) => ({ qb: { yard: losY - 5, lane: 25 },
    wrs: [{ yard: losY, lane: 6, route: 'post' }, { yard: losY - 1, lane: 38, route: 'out' },
          { yard: losY - 1, lane: 46, route: 'flat' }, { yard: losY, lane: 54, route: 'streak' }] })},
  { name: 'Levels Left', getPositions: (losY) => ({ qb: { yard: losY - 5, lane: 35 },
    wrs: [{ yard: losY, lane: 6, route: 'streak' }, { yard: losY - 1, lane: 14, route: 'flat' },
          { yard: losY - 1, lane: 22, route: 'out' }, { yard: losY, lane: 54, route: 'post' }] })},
  { name: 'Curl-Flat', getPositions: (losY) => ({ qb: { yard: losY - 5, lane: 30 },
    wrs: [{ yard: losY, lane: 6, route: 'flat' }, { yard: losY - 1, lane: 18, route: 'curl' },
          { yard: losY - 1, lane: 42, route: 'curl' }, { yard: losY, lane: 54, route: 'flat' }] })},
  { name: 'Smash Right', getPositions: (losY) => ({ qb: { yard: losY - 5, lane: 25 },
    wrs: [{ yard: losY, lane: 6, route: 'post' }, { yard: losY - 1, lane: 20, route: 'drag' },
          { yard: losY - 1, lane: 44, route: 'hitch' }, { yard: losY, lane: 54, route: 'corner' }] })},
  { name: 'Smash Left', getPositions: (losY) => ({ qb: { yard: losY - 5, lane: 35 },
    wrs: [{ yard: losY, lane: 6, route: 'corner' }, { yard: losY - 1, lane: 16, route: 'hitch' },
          { yard: losY - 1, lane: 40, route: 'drag' }, { yard: losY, lane: 54, route: 'post' }] })},
  { name: 'Dagger', getPositions: (losY) => ({ qb: { yard: losY - 5, lane: 30 },
    wrs: [{ yard: losY, lane: 6, route: 'flat' }, { yard: losY - 1, lane: 20, route: 'post' },
          { yard: losY - 1, lane: 40, route: 'dig' }, { yard: losY, lane: 54, route: 'streak' }] })},
  { name: 'Mills', getPositions: (losY) => ({ qb: { yard: losY - 5, lane: 30 },
    wrs: [{ yard: losY, lane: 6, route: 'flat' }, { yard: losY - 1, lane: 20, route: 'post' },
          { yard: losY - 1, lane: 40, route: 'dig' }, { yard: losY, lane: 54, route: 'seam' }] })},
  { name: 'Y-Cross', getPositions: (losY) => ({ qb: { yard: losY - 5, lane: 30 },
    wrs: [{ yard: losY, lane: 6, route: 'curl' }, { yard: losY - 1, lane: 20, route: 'slant' },
          { yard: losY - 1, lane: 38, route: 'dig' }, { yard: losY, lane: 54, route: 'hitch' }] })},
  { name: 'All Curls', getPositions: (losY) => ({ qb: { yard: losY - 5, lane: 30 },
    wrs: [{ yard: losY, lane: 6, route: 'curl' }, { yard: losY - 1, lane: 20, route: 'curl' },
          { yard: losY - 1, lane: 40, route: 'curl' }, { yard: losY, lane: 54, route: 'curl' }] })},
  { name: 'Wheel Route', getPositions: (losY) => ({ qb: { yard: losY - 5, lane: 30 },
    wrs: [{ yard: losY, lane: 6, route: 'wheel' }, { yard: losY - 1, lane: 20, route: 'flat' },
          { yard: losY - 1, lane: 40, route: 'hitch' }, { yard: losY, lane: 54, route: 'curl' }] })},
  { name: 'Corner Strike', getPositions: (losY) => ({ qb: { yard: losY - 5, lane: 30 },
    wrs: [{ yard: losY, lane: 6, route: 'corner' }, { yard: losY - 1, lane: 20, route: 'flat' },
          { yard: losY - 1, lane: 40, route: 'drag' }, { yard: losY, lane: 54, route: 'corner' }] })},
  { name: 'Deep Cross', getPositions: (losY) => ({ qb: { yard: losY - 5, lane: 30 },
    wrs: [{ yard: losY, lane: 6, route: 'flat' }, { yard: losY - 1, lane: 20, route: 'post' },
          { yard: losY - 1, lane: 40, route: 'post' }, { yard: losY, lane: 54, route: 'flat' }] })},
  { name: 'Double Post', getPositions: (losY) => ({ qb: { yard: losY - 5, lane: 30 },
    wrs: [{ yard: losY, lane: 6, route: 'flat' }, { yard: losY - 1, lane: 18, route: 'post' },
          { yard: losY - 1, lane: 42, route: 'post' }, { yard: losY, lane: 54, route: 'dig' }] })},
  { name: 'Spacing', getPositions: (losY) => ({ qb: { yard: losY - 5, lane: 30 },
    wrs: [{ yard: losY, lane: 6, route: 'out' }, { yard: losY - 1, lane: 20, route: 'hitch' },
          { yard: losY - 1, lane: 40, route: 'hitch' }, { yard: losY, lane: 54, route: 'out' }] })},
  { name: 'Snag', getPositions: (losY) => ({ qb: { yard: losY - 5, lane: 30 },
    wrs: [{ yard: losY, lane: 6, route: 'flat' }, { yard: losY - 1, lane: 18, route: 'corner' },
          { yard: losY - 1, lane: 38, route: 'hitch' }, { yard: losY, lane: 54, route: 'streak' }] })},
  { name: 'China', getPositions: (losY) => ({ qb: { yard: losY - 5, lane: 30 },
    wrs: [{ yard: losY, lane: 6, route: 'out' }, { yard: losY - 1, lane: 20, route: 'post' },
          { yard: losY - 1, lane: 40, route: 'drag' }, { yard: losY, lane: 54, route: 'streak' }] })},
];
...
function drawPixelPlayer(c, x, y, team, num, action, frame, isQB, scale, starAura) {
  ...
  c.font = `bold ${Math.round(10 * s)}px "Courier New", monospace`;
...
function generateFieldTexture() {
  ...
  fx.fillStyle = 'rgba(232,220,200,0.2)'; fx.font = 'bold 16px "Courier New", monospace'; fx.textAlign = 'center';
  ...
  for (let y = 10; y <= 40; y += 10) {
    ...
    fx.font = 'bold 15px "Courier New", monospace'; fx.textAlign = 'right';
  ...
  fx.font = 'bold 22px "Courier New"'; fx.textAlign = 'center';
...
function startSimulation(chosenWR) {
  ...
  const rushEntity = {
    ...
    vy: 0, vl: 0,
    maxSpeed: play.rushFast ? 6.0 : 4.5,
    accel: 3.5,
  };
  const qbEntity = {
  ...
  sim = {
    ...
    willSack: false, scrambleTriggered: false, scrambleChoice: null, scrambleTimer: 0,
...
function updateSimulation(dt) {
  ...
  switch (sim.phase) {
    ...
    case 'routes': {
      ...
      if (!sim.rushEntity) {
        sim.rushEntity = Object.assign(sim.rushPos, { vy: 0, vl: 0, maxSpeed: currentPlay.rushFast ? 6.0 : 4.5, accel: 3.5 });
      }
      physicsMove(sim.rushEntity, sim.qbPos.yard, sim.qbPos.lane, sd);
      ...
      // Dynamic sack check — if rusher physically reaches QB, trigger scramble
      const rushDistNow = Math.sqrt(Math.pow(sim.rushEntity.yard - sim.qbPos.yard, 2) + Math.pow(sim.rushEntity.lane - sim.qbPos.lane, 2));
      if ((rushDistNow < 1.5 || sim.willSack) && !sim.scrambleTriggered) {
      ...
      if (!game.waitingForPassType && game.passType) {
        ...
        if (isStopRoute) {
          ...
          // Iterate for convergence (ball target affects distance affects flight time)
          ...
          for (let iter = 0; iter < 3; iter++) {
            ...
            // WR runs at max speed for entire flight time (no toEndDist cap — WR keeps running)
            const wrRunDist = wrSpeed * flightTime;
            leadYard = wrE.yard + runDirY * wrRunDist;
            leadLane = wrE.lane + runDirL * wrRunDist;
          }
          baseYard = leadYard; baseLane = leadLane;
        }

        // Route progress affects lead accuracy (1B) — longer routes = better read
        const routeAccuracyFactor = Math.min(1.0, (sim.routeProgress - 0.4) / 0.5);
        let leadAccuracy = 1.0;
        if (game.passType === 'touch') leadAccuracy = 0.6 + 0.4 * routeAccuracyFactor;
        else if (game.passType === 'lob') leadAccuracy = 0.4 + 0.6 * routeAccuracyFactor;
        // Bullet always ~1.0 (short flight, easy to aim)

        // Rusher pressure degrades accuracy (1C)
        let pressureFactor = 1.0;
        if (rushDistNow < 2) pressureFactor = 0.3;
        else if (rushDistNow < 4) pressureFactor = 0.6;
        else if (rushDistNow < 6) pressureFactor = 0.8;
        leadAccuracy *= pressureFactor;

        // Apply accuracy error to ball target
        const leadError = (1 - leadAccuracy) * 4;
...
function drawPixelButton(c, btn, hover) {
  ...
  c.font = 'bold 15px "Courier New", monospace'; c.textAlign = 'center'; c.textBaseline = 'middle';
...
function drawWRCards(interactive) {
  ...
  for (let i = 0; i < 4; i++) {
    ...
    if (i === currentPlay.bestWR && (hasRelic('film_study') || game.scoutReport || game.filmStudyFloorsLeft > 0)) {
      ctx.fillStyle = COL.uiGold; ctx.font = '13px serif'; ctx.textAlign = 'right';
...
function generateTeachingMoment(lr) {
  ...
  if (lr.isSack) {
    ...
    if (lr.closestDBDist < 3) {
      ...
      tip = '💡 防守球员紧贴时，INT风险极高。应该读防选择更空的接球手，或者用bullet传球缩小被抄截窗口。';
    ...
    if (lr.closestDBDist > 10) {
      tip = '💡 漂亮的读防！你找到了防守中的空隙，这种wide open接球几乎不可能失误。';
    } else if (lr.closestDBDist > 5) {
      tip = '💡 不错的选择，接球手有合理的分离空间。稳定是四分卫最重要的品质。';
    } else {
      tip = '💡 勇敢的传球！但DB很近有风险，寻找更开阔的目标能提升整体效率。';
    }
    if (lr.yacYards > 5) {
      tip += ` 🏃 ${lr.wrName}接球后又推进了${lr.yacYards}码！空间越大=YAC越多。`;
    }
  } else {
    title = '📖 传球未完成分析';
    ...
    if (lr.closestDBDist < 2) {
      ...
      tip = '💡 防守紧贴时接球极难。先看防守站位，找到defensive gap露出的接球手再传。';
    } else if (lr.closestDBDist < 5) {
      lines.push('接球手有一定空间，但竞争激烈。');
      tip = '💡 竞争区域可以考虑：等路线跑完再出手、选更空的接球手、用motion识别防守。';
    } else {
      lines.push('接球手有空间但仍未完成接球。');
      tip = '💡 有时候运气也是比赛的一部分。保持信心，继续传给空位接球手。';
    }
    if (lr.coverageIsMan && (lr.route === 'slant' || lr.route === 'drag')) {
      tip += ' ⚡ 对盯人防守，slant/drag是最佳选择之一。';
    }
    if (!lr.coverageIsMan && (lr.route === 'streak' || lr.route === 'post')) {
      tip += ' 🎯 对区域防守，curl和dig路线通常能在空隙中停下来接球。';
    }
  }
...
function drawTeachingMoment() {
  ...
  const lineH = 22, tipLineH = 20, wrapW = cardW - 30;
  let totalH = 50; // title + divider
  ...
  ctx.fillText(teachingMomentData.title, W / 2, y); y += 32;
  ...
  for (const line of teachingMomentData.lines) {
    ...
    for (const ch of line) {
      if (ctx.measureText(currentLine + ch).width > wrapW) { ctx.fillText(currentLine, cardX + 15, y); currentLine = ch; y += lineH; }
      else currentLine += ch;
    }
    ctx.fillText(currentLine, cardX + 15, y); y += lineH + 4;
  }
  y += 15;
  ...
  if (teachingMomentData.tip) {
    ...
    for (const ch of teachingMomentData.tip) {
      if (ctx.measureText(currentLine + ch).width > wrapW) { ctx.fillText(currentLine, cardX + 14, y); currentLine = ch; y += tipLineH; }
      else currentLine += ch;
    }
    ctx.fillText(currentLine, cardX + 14, y); y += tipLineH + 15;
  }
...
function drawEventScreen() {
  ...
  let line = '', lineY = 120; ctx.font = '13px "Courier New"'; ctx.fillStyle = '#ccc';
...
function drawShopScreen() {
  ...
  for (let i = 0; i < shopItems.length; i++) {
    ...
    ctx.fillStyle = cb ? COL.parchment : '#555'; ctx.font = 'bold 13px "Courier New"'; ctx.textAlign = 'left';
...
function drawSeasonHighlights(isVictory) {
  ...
  ctx.save(); ctx.textAlign = 'center';
  if (isVictory) { ctx.fillStyle = COL.uiGold; ctx.font = 'bold 22px "Courier New"'; ctx.fillText('🏆 CHAMPION! 🏆', W / 2, 30); }
