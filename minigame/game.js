...
function handlePlayResult() {
  ...
  if (sim) {
    ...
    const LTOY_T = 0.42;
    // Per-DB distances and pre-snap cushions for all DBs
    const dbDistances = [];
    ...
    // Pre-snap DB alignment data from defense formation
    ...
    if (defense) {
      ...
      for (let di = 0; di < defense.dbs.length; di++) {
        ...
        // Pre-snap cushion = how far the DB was from the WR at snap
    ...
    // Find the DB covering this specific WR (for man coverage)
    let manCoverDB = null;
    ...
    // All WR scores for "better options" analysis
