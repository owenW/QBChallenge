// ============================================================
// QB CHALLENGE V13 — 3D DYNASTY MODE (POST-PROCESSING + AUDIO + SKELETAL)
...
function createStadiumLights() {
  ...
  positions.forEach(([x, y, z]) => {
    ...
    const glowMat = new THREE.MeshBasicMaterial({ color: new THREE.Color(2, 2, 1.8) }); // HDR color for bloom
...
window.addEventListener('resize', () => {
  ...
  bokehPass.uniforms.width && (bokehPass.uniforms.width.value = window.innerWidth);
  bokehPass.uniforms.height && (bokehPass.uniforms.height.value = window.innerHeight);
});


// ============================================================
// POST-PROCESSING PIPELINE (Bloom + DOF + Anti-aliasing)
// ============================================================
const composer = new THREE.EffectComposer(renderer);
const renderPass = new THREE.RenderPass(scene, camera);
composer.addPass(renderPass);

// Unreal Bloom — stadium lights, explosions, ball trail, yard markers
...
// Depth of Field (Bokeh) — hero moment throws
...
bokehPass.enabled = false; // toggled on during hero moments
composer.addPass(bokehPass);

// FXAA-like sharpening via CopyShader final pass
// (renderer already has MSAA via antialias:true; this ensures post-processing output is clean)
...
function enableDOF(focusDist) {
  ...
  ppState.dofActive = true;
}

function disableDOF() {
  bokehPass.enabled = false;
  ppState.dofActive = false;
}

function boostBloomForTD() {
  ppState.tdExplosionBoost = 1.0;
}

function updatePostProcessing(dt) {
  // TD explosion bloom boost decay
  if (ppState.tdExplosionBoost > 0) {
    ppState.tdExplosionBoost = Math.max(0, ppState.tdExplosionBoost - dt * 0.5);
    bloomPass.strength = ppState.bloomBaseStrength + ppState.tdExplosionBoost * 0.8;
  } else {
    bloomPass.strength = ppState.bloomBaseStrength;
  }
}
...
const ProceduralAudio = (() => {
  ...
  function playNoiseBurst(duration, filterFreq, filterQ, gainVal, attack, decay) {
    if (!ensureCtx()) return;
    const buf = createNoiseBuffer(duration + 0.1, 'white');
  ...
  function playTone(freq, duration, type, gainVal, attack, decay, detune) {
    if (!ensureCtx()) return;
    ...
    const now = ctx.currentTime;
    g.gain.setValueAtTime(0, now);
    g.gain.linearRampToValueAtTime(gainVal, now + (attack || 0.01));
    g.gain.exponentialRampToValueAtTime(0.001, now + (attack || 0.01) + (decay || duration));
  ...
  const sounds = {
    snap() {
      // Short percussive pop — white noise burst, 50ms
      playNoiseBurst(0.08, 2000, 1, 0.6, 0.003, 0.05);
      playTone(150, 0.05, 'sine', 0.3, 0.002, 0.04);
    },
    throw_ball() {
      // Whoosh — filtered noise sweep, 200ms
      if (!ensureCtx()) return;
      const buf = createNoiseBuffer(0.3, 'white');
      const src = ctx.createBufferSource(); src.buffer = buf;
      const filt = ctx.createBiquadFilter();
      filt.type = 'bandpass'; filt.Q.value = 3;
      const now = ctx.currentTime;
      filt.frequency.setValueAtTime(300, now);
      filt.frequency.exponentialRampToValueAtTime(3000, now + 0.15);
      filt.frequency.exponentialRampToValueAtTime(800, now + 0.25);
      const g = ctx.createGain();
      g.gain.setValueAtTime(0, now);
    ...
    catch_ball() {
      // Satisfying thud — low freq + noise, 100ms
      playTone(80, 0.12, 'sine', 0.5, 0.005, 0.1);
      playNoiseBurst(0.08, 800, 2, 0.25, 0.005, 0.06);
    },
    drop() {
      // Dull thud + bounce — lower pitch, longer decay
      playTone(55, 0.2, 'sine', 0.35, 0.01, 0.18);
      playNoiseBurst(0.15, 400, 1.5, 0.15, 0.01, 0.12);
      setTimeout(() => { playTone(45, 0.1, 'sine', 0.15, 0.01, 0.08); }, 180);
    },
    sack() {
      // Heavy impact — low freq + distortion
      if (!ensureCtx()) return;
      ...
      playNoiseBurst(0.2, 300, 0.5, 0.4, 0.005, 0.18);
    },
    touchdown() {
      // Ascending fanfare — chord progression, 1s
      if (!ensureCtx()) return;
      const notes = [
        {f:261.6, t:0, d:0.25},   // C
        {f:329.6, t:0.15, d:0.25}, // E
        {f:392.0, t:0.3, d:0.3},   // G
        {f:523.3, t:0.5, d:0.5},   // C5
        {f:659.3, t:0.5, d:0.5},   // E5
      ];
      notes.forEach(n => {
        setTimeout(() => {
          playTone(n.f, n.d + 0.3, 'triangle', 0.2, 0.01, n.d + 0.2);
          playTone(n.f * 2, n.d + 0.2, 'sine', 0.08, 0.02, n.d + 0.1);
        }, n.t * 1000);
      });
      // Cymbal crash
      setTimeout(() => playNoiseBurst(0.8, 6000, 0.5, 0.2, 0.005, 0.7), 500);
    },
    startCrowd() {
      // Continuous low white noise, persistent
      if (!ensureCtx()) return;
      if (crowdSource) return;
      const buf = createNoiseBuffer(4, 'pink');
      crowdSource = ctx.createBufferSource(); crowdSource.buffer = buf; crowdSource.loop = true;
      ...
      crowdSource.connect(filt); filt.connect(crowdGain); crowdGain.connect(masterGain);
      crowdSource.start();
    },
    setCrowdLevel(level) {
      // level 0-1: quiet to roaring
      if (crowdGain) {
        crowdGain.gain.linearRampToValueAtTime(0.04 + level * 0.18, (ctx?.currentTime || 0) + 0.2);
      }
    },
    whistle() {
      // High pitched oscillator — referee whistle
      if (!ensureCtx()) return;
      const now = ctx.currentTime;
      playTone(2800, 0.5, 'sine', 0.2, 0.01, 0.45);
      playTone(2850, 0.5, 'sine', 0.15, 0.01, 0.45);
      // Vibrato via second oscillator
      playTone(2780, 0.45, 'sine', 0.1, 0.02, 0.4);
    },
    menuClick() {
      // Short blip
      playTone(880, 0.06, 'sine', 0.15, 0.005, 0.04);
      playTone(1320, 0.04, 'sine', 0.08, 0.005, 0.03);
    },
    heroMoment() {
      // Dramatic low rumble with rising pitch
      if (!ensureCtx()) return;
      ...
      g.gain.linearRampToValueAtTime(0.15, now + 0.7);
      g.gain.exponentialRampToValueAtTime(0.001, now + 0.9);
      const filt = ctx.createBiquadFilter(); filt.type = 'lowpass';
      filt.frequency.setValueAtTime(100, now);
      filt.frequency.exponentialRampToValueAtTime(800, now + 0.8);
      osc.connect(filt); filt.connect(g); g.connect(masterGain);
      osc.start(now); osc.stop(now + 1.0);
      // Sub bass
      playTone(30, 0.8, 'sine', 0.25, 0.05, 0.7);
    },
    interception() {
      // Descending tone — bad outcome
      if (!ensureCtx()) return;
      const osc = ctx.createOscillator(); osc.type = 'sawtooth';
      const now = ctx.currentTime;
    ...
    firstDown() {
      // Positive chime
      playTone(523.3, 0.15, 'triangle', 0.2, 0.005, 0.12);
      setTimeout(() => playTone(659.3, 0.12, 'triangle', 0.18, 0.005, 0.1), 100);
      setTimeout(() => playTone(784.0, 0.2, 'triangle', 0.22, 0.005, 0.18), 200);
    },
  };
  ...
  return {
    ...
    startCrowd() { sounds.startCrowd(); },
    setCrowdLevel(l) { sounds.setCrowdLevel(l); },
    setVolume(v) { volume = v; if (masterGain) masterGain.gain.value = muted ? 0 : v; },
    toggleMute() {
      muted = !muted;
...
function createField() {
  ...
  const lineMat = new THREE.MeshBasicMaterial({ color: new THREE.Color(1.2, 1.2, 1.2) }); // slight bloom on yard lines
...
// ============================================================
// 3D PLAYER — SKELETAL ANIMATION SYSTEM
// Uses THREE.Bone + THREE.Skeleton + THREE.SkinnedMesh
// ============================================================

// Bone indices for the skeleton
const BONE = {
  HIPS: 0, SPINE: 1, CHEST: 2, HEAD: 3,
  L_UPPER_ARM: 4, L_FOREARM: 5, L_HAND: 6,
  R_UPPER_ARM: 7, R_FOREARM: 8, R_HAND: 9,
  L_THIGH: 10, L_SHIN: 11, L_FOOT: 12,
  R_THIGH: 13, R_SHIN: 14, R_FOOT: 15,
};

// Helper: merge body part geometries with skin weights
function mergeBodyGeometries(parts) {
  let totalVerts = 0;
  const processed = [];
  for (const part of parts) {
    let geo = part.geo.clone();
    if (geo.index) geo = geo.toNonIndexed();
    geo.computeVertexNormals();
    const m4 = new THREE.Matrix4();
    const pos = part.pos || new THREE.Vector3();
    const quat = new THREE.Quaternion();
    if (part.rot) quat.setFromEuler(part.rot);
    const scl = part.scl || new THREE.Vector3(1,1,1);
    ...
    processed.push({
      ...
      boneIdx: part.boneIdx,
      matIdx: part.matIdx || 0,
    });
    totalVerts += geo.attributes.position.count;
    geo.dispose();
  }
  processed.sort((a, b) => a.matIdx - b.matIdx);
  ...
  let off = 0;
  const groups = [];
  let curMat = -1, grpStart = 0;
  for (const p of processed) {
    if (p.matIdx !== curMat) {
      if (curMat !== -1) groups.push({ start: grpStart, count: off - grpStart, materialIndex: curMat });
      curMat = p.matIdx; grpStart = off;
    }
    positions.set(p.positions, off * 3);
    normals.set(p.normals, off * 3);
    for (let i = 0; i < p.count; i++) {
      skinIndices[(off + i) * 4] = p.boneIdx;
      skinWeights[(off + i) * 4] = 1.0;
    }
    off += p.count;
  }
  if (curMat !== -1) groups.push({ start: grpStart, count: off - grpStart, materialIndex: curMat });
...
function createPlayerSkeleton(H, W) {
  const bones = [];
  const boneData = [
    { name: 'legGroup',  parent: -1, pos: [0, H*0.37, 0] },         // 0: hips
    { name: 'spine',     parent: 0,  pos: [0, H*0.13, 0] },         // 1: spine
    { name: 'armGroup',  parent: 1,  pos: [0, H*0.10, 0] },         // 2: chest
    { name: 'head',      parent: 2,  pos: [0, H*0.18, 0] },         // 3: head
    { name: 'leftArm',   parent: 2,  pos: [-0.48*W, -H*0.01, 0] },  // 4: L upper arm
    { name: 'lForearm',  parent: 4,  pos: [0, -H*0.14, 0] },        // 5: L forearm
    { name: 'leftHand',  parent: 5,  pos: [0, -H*0.11, 0] },        // 6: L hand
    { name: 'rightArm',  parent: 2,  pos: [0.48*W, -H*0.01, 0] },   // 7: R upper arm
    { name: 'rForearm',  parent: 7,  pos: [0, -H*0.14, 0] },        // 8: R forearm
    { name: 'rightHand', parent: 8,  pos: [0, -H*0.11, 0] },        // 9: R hand
    { name: 'leftLeg',   parent: 0,  pos: [-0.15, -H*0.04, 0] },    // 10: L thigh
    { name: 'lShin',     parent: 10, pos: [0, -H*0.14, 0] },        // 11: L shin
    { name: 'lFoot',     parent: 11, pos: [0, -H*0.13, 0.02] },     // 12: L foot
    { name: 'rightLeg',  parent: 0,  pos: [0.15, -H*0.04, 0] },     // 13: R thigh
    { name: 'rShin',     parent: 13, pos: [0, -H*0.14, 0] },        // 14: R shin
    { name: 'rFoot',     parent: 14, pos: [0, -H*0.13, 0.02] },     // 15: R foot
  ];
  ...
  for (let i = 0; i < boneData.length; i++) {
    if (boneData[i].parent >= 0) bones[boneData[i].parent].add(bones[i]);
  }
...
function createPlayer(config) {
  ...
  const materials = [jerseyMat, pantsMat, skinMat];

  // --- Build skeleton ---
  const { bones, skeleton, rootBone } = createPlayerSkeleton(H, W);

  // --- Compute bind-pose world positions for geometry placement ---
  // We need the world Y of each bone to position geometry correctly
  const boneWorldY = [];
  const boneWorldX = [];
  function computeBoneWorld(bone, parentY, parentX) {
    const wy = parentY + bone.position.y;
    const wx = parentX + bone.position.x;
    boneWorldY.push(wy);
    boneWorldX.push(wx);
    for (const child of bone.children) {
      if (child.isBone) computeBoneWorld(child, wy, wx);
    }
  }
  // Reset: traverse in bone order
  const worldPos = [];
  function getWorldPos(boneIdx) {
  ...
  for (let i = 0; i < bones.length; i++) worldPos.push(getWorldPos(i));

  // --- Body part geometries ---
  const segs = 6; // low-poly segments
  const bodyParts = [];

  // Torso (spine region) — mat 0 (jersey)
  bodyParts.push({
    geo: new THREE.CylinderGeometry(0.35*W, 0.30*W, H*0.13, segs),
    pos: new THREE.Vector3(0, (worldPos[0].y + worldPos[1].y)/2, 0),
    boneIdx: BONE.SPINE, matIdx: 0,
  });
  // Upper torso (chest) — mat 0
  bodyParts.push({
    geo: new THREE.CylinderGeometry(0.38*W, 0.35*W, H*0.10, segs),
    pos: new THREE.Vector3(0, (worldPos[1].y + worldPos[2].y)/2, 0),
    boneIdx: BONE.CHEST, matIdx: 0,
  });

  // Left upper arm — mat 0 (jersey)
  bodyParts.push({
    geo: new THREE.CylinderGeometry(0.10, 0.09, H*0.14, segs),
    pos: new THREE.Vector3(worldPos[4].x, worldPos[4].y - H*0.05, 0),
    boneIdx: BONE.L_UPPER_ARM, matIdx: 0,
  });
  // Right upper arm — mat 0
  bodyParts.push({
    geo: new THREE.CylinderGeometry(0.10, 0.09, H*0.14, segs),
    pos: new THREE.Vector3(worldPos[7].x, worldPos[7].y - H*0.05, 0),
    boneIdx: BONE.R_UPPER_ARM, matIdx: 0,
  });

  // Left thigh — mat 1 (pants)
  bodyParts.push({
    geo: new THREE.CylinderGeometry(0.13, 0.11, H*0.14, segs),
    pos: new THREE.Vector3(worldPos[10].x, worldPos[10].y - H*0.05, 0),
    boneIdx: BONE.L_THIGH, matIdx: 1,
  });
  // Right thigh — mat 1
  bodyParts.push({
    geo: new THREE.CylinderGeometry(0.13, 0.11, H*0.14, segs),
    pos: new THREE.Vector3(worldPos[13].x, worldPos[13].y - H*0.05, 0),
    boneIdx: BONE.R_THIGH, matIdx: 1,
  });
  // Left shin — mat 1
  bodyParts.push({
    geo: new THREE.CylinderGeometry(0.10, 0.08, H*0.13, segs),
    pos: new THREE.Vector3(worldPos[11].x, worldPos[11].y - H*0.05, 0),
    boneIdx: BONE.L_SHIN, matIdx: 1,
  });
  // Right shin — mat 1
  bodyParts.push({
    geo: new THREE.CylinderGeometry(0.10, 0.08, H*0.13, segs),
    pos: new THREE.Vector3(worldPos[14].x, worldPos[14].y - H*0.05, 0),
    boneIdx: BONE.R_SHIN, matIdx: 1,
  });

  // Left forearm — mat 2 (skin)
  bodyParts.push({
    geo: new THREE.CylinderGeometry(0.08, 0.07, H*0.11, segs),
    pos: new THREE.Vector3(worldPos[5].x, worldPos[5].y - H*0.04, 0),
    boneIdx: BONE.L_FOREARM, matIdx: 2,
  });
  // Right forearm — mat 2
  bodyParts.push({
    geo: new THREE.CylinderGeometry(0.08, 0.07, H*0.11, segs),
    pos: new THREE.Vector3(worldPos[8].x, worldPos[8].y - H*0.04, 0),
    boneIdx: BONE.R_FOREARM, matIdx: 2,
  });
  // Left hand — mat 2
  bodyParts.push({
    geo: new THREE.SphereGeometry(0.08, segs, 4),
    pos: new THREE.Vector3(worldPos[6].x, worldPos[6].y, 0),
    boneIdx: BONE.L_HAND, matIdx: 2,
  });
  // Right hand — mat 2
  bodyParts.push({
    geo: new THREE.SphereGeometry(0.08, segs, 4),
    pos: new THREE.Vector3(worldPos[9].x, worldPos[9].y, 0),
    boneIdx: BONE.R_HAND, matIdx: 2,
  });

  // --- Merge into SkinnedMesh ---
  const bodyGeo = mergeBodyGeometries(bodyParts);
  const bodyMesh = new THREE.SkinnedMesh(bodyGeo, materials);
  ...
  for (const fIdx of [BONE.L_FOOT, BONE.R_FOOT]) {
    const cleatGeo = new THREE.BoxGeometry(0.18, 0.07, 0.26);
    const cleat = new THREE.Mesh(cleatGeo, SharedMaterials.black);
    cleat.position.set(0, -0.02, 0.03);
    cleat.castShadow = true;
    bones[fIdx].add(cleat);
  }
  ...
  const stripe = new THREE.Mesh(
    new THREE.BoxGeometry(0.025, 0.4, 0.05),
  ...
  // Visor
  if (hasVisor) {
    const visorGeo = new THREE.SphereGeometry(helmR - 0.02, 10, 5, -Math.PI*0.4, Math.PI*0.8, 0, Math.PI*0.35);
    const visorMat = new THREE.MeshStandardMaterial({
      color: visorColor, transparent: true, opacity: 0.5, roughness: 0.1, metalness: 0.3,
    });
    const visor = new THREE.Mesh(visorGeo, visorMat);
    visor.position.set(0, H*0.07, 0.05);
    visor.rotation.x = -0.3;
    bones[BONE.HEAD].add(visor);
  }

  // Headband
  if (hasHeadband) {
    const hb = new THREE.Mesh(
      new THREE.TorusGeometry(helmR + 0.02, 0.022, 4, 18),
      new THREE.MeshStandardMaterial({ color: 0xffffff, roughness: 0.5 })
    );
    hb.position.set(0, H*0.08, 0);
    hb.rotation.x = Math.PI / 2;
    bones[BONE.HEAD].add(hb);
  }

  // Gloves
  if (hasGloves) {
    for (const hIdx of [BONE.L_HAND, BONE.R_HAND]) {
      const glove = new THREE.Mesh(
        new THREE.SphereGeometry(0.09, segs, 4),
        new THREE.MeshStandardMaterial({ color: gloveColor, roughness: 0.5 })
      );
      glove.position.set(0, 0, 0);
      bones[hIdx].add(glove);
    }
  }
  ...
  if (isQB) {
    const chin = new THREE.Mesh(
      new THREE.TorusGeometry(0.06, 0.010, 4, 8, Math.PI),
      SharedMaterials.gold
    );
    chin.position.set(0, -0.02, helmR*0.5);
    chin.rotation.z = Math.PI;
    bones[BONE.HEAD].add(chin);
    ...
    cPatch.position.set(0.28*W, -H*0.02, 0.30*W);
    bones[BONE.CHEST].add(cPatch);
  }
  ...
  if (accentColor) {
    for (const side of [-1, 1]) {
      const accent = new THREE.Mesh(
        new THREE.BoxGeometry(0.015, 0.13, 0.18),
        new THREE.MeshStandardMaterial({ color: accentColor, roughness: 0.3, metalness: 0.2 })
      );
      accent.position.set(side * (helmR - 0.02), H*0.06, 0.05);
      bones[BONE.HEAD].add(accent);
    }
  }
  ...
  if (number) {
    ...
    const numTex = new THREE.CanvasTexture(numCanvas);
    const numMat = new THREE.MeshBasicMaterial({ map: numTex, transparent: true, side: THREE.DoubleSide });
    const numGeo = new THREE.PlaneGeometry(0.50, 0.50);
    const numFront = new THREE.Mesh(numGeo, numMat);
    numFront.position.set(0, -H*0.05, 0.32*W + 0.01);
    bones[BONE.CHEST].add(numFront);
    const numBack = new THREE.Mesh(numGeo, numMat.clone());
    numBack.position.set(0, -H*0.05, -(0.32*W + 0.01));
    numBack.rotation.y = Math.PI;
    bones[BONE.CHEST].add(numBack);
  }

  // Shadow circle on ground
  const shadowGeo = new THREE.CircleGeometry(0.55 * W, 16);
  ...
  if (accentColor || hasVisor || hasGloves || hasHeadband) {
    ...
    indicator.position.set(0, H*0.16, 0);
    bones[BONE.HEAD].add(indicator);
  }
  ...
  group.userData = {
    ...
    throwPhase: 0,
    isAnimating: false,
    bones, // direct bone references for animation
    skeleton,
    bodyMesh,
  };
  return group;
...
// These work with the named-bone system for backward compatibility
// getObjectByName traversal finds bones by name through the hierarchy
// ============================================================

function animatePlayerRun(player, dt, speed) {
...
function animatePlayerBlock(player, time) {
  const b = player.userData.bones;
  if (!b) return;
  // Wide stance, arms up and out
  b[BONE.L_THIGH].rotation.x = -0.15;
  b[BONE.R_THIGH].rotation.x = -0.15;
  b[BONE.L_SHIN].rotation.x = 0.2;
  b[BONE.R_SHIN].rotation.x = 0.2;
  b[BONE.SPINE].rotation.x = 0.15;
  b[BONE.R_UPPER_ARM].rotation.x = -0.5 + Math.sin(time * 3) * 0.1;
  b[BONE.R_UPPER_ARM].rotation.z = 0.6;
  b[BONE.L_UPPER_ARM].rotation.x = -0.5 + Math.sin(time * 3 + 1) * 0.1;
  b[BONE.L_UPPER_ARM].rotation.z = -0.6;
  b[BONE.R_FOREARM].rotation.x = -0.6;
  b[BONE.L_FOREARM].rotation.x = -0.6;
}

function animatePlayerRush(player, dt) {
  const data = player.userData;
  data.runPhase += dt * 10;
  const b = data.bones;
  if (!b) return;
...
function addBallTrail(pos, color = 0xffffff) {
  ...
  const mat = new THREE.MeshBasicMaterial({ color, transparent: true, opacity: 0.6, toneMapped: false }); // toneMapped:false = blooms
  const mesh = new THREE.Mesh(geo, mat);
  mesh.position.copy(pos);
  scene.add(mesh);
...
function updateSimulation(dt) {
  ...
  switch (sim.phase) {
    case 'snap': {
      if (sim.timer === 0) ProceduralAudio.play('snap');
    ...
    case 'dropback': {
      ...
      // QB backpedal animation using bones
      if (playerObjects.qb.userData.bones) {
        const b = playerObjects.qb.userData.bones;
        b[BONE.R_THIGH].rotation.x = Math.sin(sim.timer * 12) * 0.3;
        b[BONE.L_THIGH].rotation.x = -Math.sin(sim.timer * 12) * 0.3;
        b[BONE.R_SHIN].rotation.x = Math.max(0, Math.sin(sim.timer * 12 + 0.5)) * 0.2;
        b[BONE.L_SHIN].rotation.x = Math.max(0, Math.sin(sim.timer * 12 + 0.5 + Math.PI)) * 0.2;
      }
    ...
    case 'routes':
      ...
      if (sim.routeProgress >= 0.7) {
        ...
        ProceduralAudio.play('throw_ball');
        ProceduralAudio.play('heroMoment');
    ...
    case 'sackResult':
      animatePlayerFall(playerObjects.qb, sim.timer / 1.5);
    ...
    case 'throw': {
      ...
      if (sim.throwProgress >= 1) {
        ...
        if (sim.success) {
          ProceduralAudio.play('catch_ball');
          ProceduralAudio.setCrowdLevel(0.6);
          ...
          ProceduralAudio.play('interception');
          ProceduralAudio.setCrowdLevel(0.2);
          ...
          ProceduralAudio.play('drop');
          ProceduralAudio.setCrowdLevel(0.3);
    ...
    case 'catch':
      ...
      if (sim.timer > 1.2) {
        ...
        if (isTD) {
          ...
          ProceduralAudio.play('touchdown');
          ProceduralAudio.setCrowdLevel(1.0);
          boostBloomForTD();
    ...
    case 'tdCelebration':
      ...
      // Celebration animations using skeletal system
      playerObjects.wrs.forEach((wr, i) => {
        wr.position.y = Math.abs(Math.sin(game.time * 5 + i * 1.5)) * 1.0;
        wr.rotation.y = game.time * 2 + i;
        animatePlayerCelebrate(wr, game.time + i * 0.3);
      });
      // QB fist pump
      playerObjects.qb.position.y = Math.abs(Math.sin(game.time * 4)) * 0.6;
      animatePlayerCelebrate(playerObjects.qb, game.time);
...
function handlePlayResult() {
  ProceduralAudio.play('whistle');
  ProceduralAudio.setCrowdLevel(0.15);
  hideResultOverlay();
  ...
  if (sim.isSack) {
    ...
    if (game.ballYardLine >= 50) {
      ...
      if (!game.gotFirstDown && game.ballYardLine >= game.firstDownLine) {
        ...
        ProceduralAudio.play('firstDown');
...
function enterChoosingPhase() {
  ...
  for (let i = 0; i < 4; i++) {
    ...
    card.onclick = () => {
      ProceduralAudio.play('menuClick');
      wrSelectEl.classList.add('hidden');
      startSimulation(i);
    };
...
window._setVolume = function(val) {
  ProceduralAudio.init();
  ProceduralAudio.setVolume(val / 100);
};
