...
function drawScoreBug() {
  const bH = 60, bY = H - bH - 34, bX = 6, bW = W - 12;
  ctx.save();
...
function drawPoiseRating() {
  ...
  const pw = 64, px = W - pw - 6, py = 95, col = colors[level];
...
function drawRelicsBar() {
  ...
  for (let i = 0; i < relics.length; i++) {
    drawPixelRect(ctx, 4 + i * 18, 93, 16, 16, 'rgba(42,34,28,0.8)', 'rgba(138,118,80,0.4)');
    ctx.font = '14px serif'; ctx.textAlign = 'center'; ctx.textBaseline = 'middle';
    ctx.fillText(relics[i].icon, 12 + i * 18, 102);
  }
  ctx.restore();
}
...
function drawTopBar() {
  ...
  if (currentPlay) {
    ...
    ctx.fillText(currentPlay.offense.name, 10, 33);
  ...
  ctx.fillText(`${wins}W${ties ? '-' + ties + 'T' : ''}-${losses}L`, W - 10, 61);
  ctx.restore();
}
...
function drawWRCards(interactive) {
  if (!currentPlay) return;
  const cW = 108, cH = 148, totalW = 4 * cW + 3 * 4, startX = (W - totalW) / 2, baseY = H - cH - 92;
  cardButtons = [];
  for (let i = 0; i < 4; i++) {
...
function drawSeasonHighlights(isVictory) {
  ...
  if (!Leaderboard.submitted && !Leaderboard._prompted) {
    ...
    setTimeout(() => {
      const saved = localStorage.getItem('qb_player_id') || '';
      const name = prompt('输入你的ID（排行榜显示）:', saved);
      if (name && name.trim()) {
        Leaderboard.playerName = name.trim().slice(0, 12);
        localStorage.setItem('qb_player_id', Leaderboard.playerName);
