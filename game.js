// ============================================================
// QB CHALLENGE V14 — PIXEL ART ROGUELIKE
...
const game = {
  ...
  firstDownLine: 25, gotFirstDown: false, score: 0,
...
function startSimulation(chosenWR) {
  ...
  let willSack = play.rushFast && Math.random() < (0.35 * rushFactor);
...
function handlePlayResult() {
  ...
  if (sim.isSack) {
    ...
    if (game.ballYardLine >= 50) {
      ...
      // Check for halftime (after 6 plays)
...
function endCurrentGame(won) {
  const team = getCurrentTeam();
  game.seasonRecord.push({ teamId: team.id, won, playerScore: game.gameScore.player, oppScore: game.gameScore.opponent });
  if (!won) game.losses++;
...
function drawTopBar() {
  ...
  const losses = game.seasonRecord.filter(r => !r.won).length;
  ctx.fillText(`${wins}W-${losses}L`, W - 10, 46);
  ctx.restore();
}
...
function drawTitle(dt) {
  ...
  ctx.fillStyle = COL.uiGold; ctx.font = 'bold 8px "Courier New"'; ctx.fillText('V14', W - 25, 19);
  ctx.restore();
  ...
  ctx.fillText('V14 Pixel Art Roguelike · 12 Teams · Star Players · 25+ Relics', W / 2, H - 20);
  drawParticles();
}
...
function drawSeasonMapScreen() {
  ...
  const wins = game.seasonRecord.filter(r => r.won).length;
  const ls = game.seasonRecord.filter(r => !r.won).length;
  ctx.fillStyle = COL.uiGold; ctx.font = '9px "Courier New"';
  ctx.fillText(`第${game.gameNum}场 · ${wins}胜${ls}负 · 💰${game.gold}`, W / 2, 38);
  ...
  for (let g = 0; g < 10; g++) {
    ...
    if (isNext) {
      ...
      if (result) {
        ...
        ctx.fillText(result.won ? `W ${result.playerScore}-${result.oppScore}` : `L ${result.playerScore}-${result.oppScore}`, nodeX + nodeW - 8, ny + 4);
      }
    } else {
  ...
  ctx.fillText(`败场: ${game.losses}/${game.maxLosses}（3败赛季结束）`, W / 2, H - 20);
...
function drawPassTypeScreen(dt) {
  ...
  const timeLimit = hasRelic('time_freeze') ? 999 : 2.0;
  ...
  if (!hasRelic('time_freeze')) {
    ...
    ctx.fillStyle = timeLeft / 2.0 > 0.3 ? COL.uiAccent : COL.uiRed;
    ctx.fillRect(40, H / 2 - 60, (W - 80) * (timeLeft / 2.0), 4);
  }
  ctx.fillStyle = COL.parchment; ctx.font = 'bold 12px "Courier New"'; ctx.textAlign = 'center';
...
function drawSeasonHighlights(isVictory) {
  ...
  const wins = game.seasonRecord.filter(r => r.won).length, ls = game.seasonRecord.filter(r => !r.won).length;
  ctx.fillText(`${wins}W ${ls}L | ${game.seasonStats.completions}/${game.seasonStats.attempts} ${game.seasonStats.yards}码 ${game.seasonStats.tds}TD ${game.seasonStats.ints}INT`, W / 2, 65);
